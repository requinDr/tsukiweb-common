...
function parseEntries(buffer: Buffer, fileCount: number, baseOffset: number) {
...
export async function extractSar(
  ...
  try {
    ...
    for (let i = 0; i < totalToExtract; i++) {
      ...
      logger.progress(`Extracting file: ${i + 1}/${totalToExtract} (${filename})`)
    ...
    logger.progress(`Extraction complete: ${extractedFiles}/${totalToExtract} files extracted to "${outputDir}"\n`)

  } catch (error) {
    logger.error(`An error occurred during extraction: ${(error as Error).message}`)
  }
}
